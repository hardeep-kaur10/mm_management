...
def STAFF_APPLY_LEAVE_SAVE(request):
    if request.method == "POST":
        ...
        for leave in leaves:
            leave_from_date = datetime.strptime(str(leave.date), '%Y-%m-%d').date()
            leave_to_date = datetime.strptime(str(leave.to_date), '%Y-%m-%d').date()
...
def attendance_report(request):
    ...
    return render(request, 'staff/attendance_report.html', {'report_data': report_data,  'date': attendance.attendance_date  })
